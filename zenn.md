...
ドメインコンポーネントの命名でAPIの用語体系と合わせたいとき、いくつかの具体的な方法があります。APIレスポンス（JSON）を見せてもらい、データがどんな名前で管理されているか確認する。SwaggerやGraphQLのスキーマを閲覧し、エンティティ名やフィールド名を把握する。AIに「うちのコードベースではこのデータは何という変数名で扱われていますか？」と聞く。命名で悩む時間を減らし、コードとデザインの語彙を最初から揃えることで、翻訳コストのない設計が可能になります。
...
増やすのは安全、変えるのは大変。ソフトウェア設計ではこれを開放閉鎖の原則（OCP: Open-Closed Principle）と呼びます。新バリアント追加＝拡張に開いている、既存プロパティ変更＝修正に閉じるべき。この原則を知っておくと、変更の影響範囲を判断する共通言語になります。
...
Figmaとコードの構造を揃える究極の理由は、ツールの一致ではなく認知の一致——チームの脳内モデルを一致させることにあります。コンポーネント名、プロパティの型、レイヤーの階層がFigmaとコードで対応していれば、会議で「あのコンポーネントのあのプロパティ」と言ったとき、全員が同じものを思い浮かべられます。この「メンタルモデルの同期」こそが、デザインシステムがコミュニケーションコストを下げる本質的なメカニズムです。
