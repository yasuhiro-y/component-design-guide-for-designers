...
コンポーネントの設計論の序盤になぜこのような俯瞰的なトピックを扱うのか。それは、コンポーネントの運用が、土台となるデザインシステムの方式に大きく規定されるからです。
...
汎用ライブラリはあらゆるサービスで使えるように設計されているため、ボタンひとつとっても膨大なプロパティが用意されています。たとえばブランドガイドラインでプライマリボタンの色が決まっていても、ライブラリ上では`colorScheme`に好きな色を指定できてしまいます。`size`もxsからxlまで5段階ありますが、自社で使うのは2つだけです。`variant`も10種類ありますが、プロダクトで許容しているのは3種類です。
...
なんとなく作るのではなく、短期・長期両面でリソースと目的を天秤にかけ、ライブラリに頼る領域と自社の独自性を追求する領域の線引きをなるべく初期に議論しておきたいところです。後になるほど方針転換のコストが大きくなるためです。
...
すでにデザインシステムなしで開発が進んでいるプロダクトに、後からシステムを導入するケースのほうがむしろ一般的であり、ゼロから始める場合とは異なる難しさがあります。

なぜ後発導入が難しいのかを理解しておくと、戦略を立てやすくなります。ゼロから作るプロダクトなら最初からコンポーネントを揃えられますが、稼働中のサービスにはすでに動いているUIが資産として存在しています。一度に変えるとQA（品質確認）の範囲が膨大になり、リグレッション（意図しない既存機能の破壊）が起きやすくなります。
...
ボタン、カード、モーダル。同じ名前のコンポーネントなのに、デザイナーとエンジニアの頭の中にある形が違う。そんなすれ違いの多くは分割の基準が策定・共有されていないことから生まれます。
...
Figmaとコードの一致性を目指すなら、コンポーネントの命名・ディレクトリ・責務範囲を後から変更するコストは大きくなりがちです。デザイナー同士の連携、エンジニアとの連携、利用箇所の改修など、影響範囲は利用箇所が増えるほど広がります。
...
では、何をコンポーネント化すると組織が最も恩恵を受けられるのでしょうか。判断の軸としては、一箇所を直せば全部直る状態にしたい場合や、連続性をもって拡張したい場合にコンポーネント化します。

コンポーネント化されているということは、そのコンポーネントを拡張できるだけでなく、類似するUIもそのコンポーネントの設計に沿って作るという道を開いていることです。つまり、単なる1画面に登場するUI以上に、組織的な合意や規範が詰まっていると捉えられます。
...
Atomic Design、FSD、フラット配置と3つのアプローチを見てきました。続いて、Feature-Sliced Designにも内包されていた汎用とドメインという分類軸について、より詳しく検討します。そのコンポーネントが汎用的（generic）なものか、ドメイン（domain）に関連するものか（Feature Component、Business Component と呼ばれることもあります）という分類です。
...
iOS/Androidそれぞれのネイティブ言語で作る方法と、[React Native](https://reactnative.dev/)やFlutterのようなクロスプラットフォームのフレームワークで一度に両方を作る方法です。どちらを選ぶかはエンジニアリングチームの判断による部分が大きいものの、デザイナーとして知っておくべきポイントがあります。

いずれの構築方法でも、ネイティブ要素が存在します。実装で悩みがちなのが、iOSやAndroidのネイティブ要素をどこまで活かすかという問題です。
...
フロントエンドやモバイルの実装では、コンポーネント名でコンポーネントを呼び出します。最初につけた名前がそのままコードに入り、ドキュメントに引用され、チームの会話に定着します。名前を変更するということは、すべての呼び出し箇所を修正するということです。後から変えるコストは思った以上に高いので、一度決めたらなるべく変えないぐらいの気持ちで設計します。
...
もうひとつ実務で役立つ視点があります。たとえば、更新日: 2026/01/01 と表示するUIがあるとき、Figma上のプロパティとしては `2026/01/01` のテキストだけを持たせ、「更新日」というラベルはコンポーネント内に固定しておくと、実装との対応が明確になります。
...
直交性が保たれていると、プロパティを追加するたびに既存の組み合わせを検証し直す必要がありません。逆に、プロパティ間に暗黙の依存があると、機能を足すたびに優先順位の整理が必要になり、そのルールは時間とともにチーム内で忘れられていきます。
...
コード側では「サブタイトルのテキスト」というプロパティが1つあれば十分です。テキストが渡されれば表示し、渡されなければ領域ごと消える。1つのプロパティで中身と表示/非表示を同時に制御できます。
...
変更を提案するときに「この変更、実装側ではどれくらいの影響がありますか？」とエンジニアに率直に聞く。それだけでチームの信頼関係は強化されます。
...
90%の定型と10%の例外を最初から織り込んでおくくらいがちょうどよいでしょう。FigmaとコードのUIが完全一致している必要はありません。インターフェース（プロパティの名前と型）が一致していることのほうが、チームの共通理解にとってはるかに重要です。
...
「もう使わないでね」という意思表示をシステムに組み込んでおきます。Figmaなら、コンポーネント名の先頭に [Deprecated]（非推奨：もう使わないでねという印）をつけたり、説明欄に代わりの NewButton を明記したり、背景色を目立つ色にする方法があります。コード側でも同様の非推奨マークを付けることで、使おうとしたときに警告が出るしくみがあります。
...
デザインするとき、データが完璧に入った理想的な状態を中心に描くのは自然なことです。しかし、コンポーネントを実装するときに必要になるのは、すべての状態の定義です。状態の設計が抜けていると、リリース直前にエンジニアから「この状態のデザインがない」と質問が届き、その分のスケジュールや工数が確保されていないということがしばしば起きます。
...
壊れにくいコンポーネントの設計原則をひとことで言えば、「何が来ても受け止めるが、外には予測可能なものだけを出す」です。これはシステム設計でいう「受け取るものには寛容に、送り出すものには厳密に」（ポステルの法則）と同じ発想です。

どんな入力（長いテキスト、空データ、想定外の画像比率）が来ても壊れない。一方で外に提供するレイアウト——marginを持たない、幅の振る舞いが予測可能——は厳密にする。この章の設計判断の多くは「入力に寛容、出力に厳密」の実践です。
...
変更のしやすさ、拡張の余地、認知の負荷。目に見えない構造に意識を向ける。これはシステム思考と呼ばれるものの一端です。
