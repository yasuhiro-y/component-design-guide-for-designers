...
これらは独立したチェックリストではなく、互いに補強し合い、ときにトレードオフを生む体系です。たとえば、制約は判断の負荷を下げますが、強すぎると合成の自由度を損ないます。カプセル化は複雑さを隠しますが、隠しすぎると慣習の形成を妨げます。こうした絶対的な正解のなさがデザインシステムを難しくしている要因であり、奥深さです。

まず6つの原則を一覧します。

- **間接化** — 一箇所を直せば全部直る仕組みをつくる
- **カプセル化** — 複雑さを隠し、使う側にはシンプルな見た目だけを見せる
- **制約** — 選択肢を意図的に絞り、迷う場面を減らす
- **意味の符号化** — 見た目ではなく意味で名付ける
- **合成** — 小さな部品を組み合わせて大きなUIを構築する
- **慣習** — ルールを明文化し、誰でも安全に拡張できる土台をつくる

これらの原則に加え、後半では以下のテーマも扱います。
