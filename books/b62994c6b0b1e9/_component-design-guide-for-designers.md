...
原則を押さえたところで、ここまでの話を裏側から見てみます。デザインシステムがない、あるいは機能していない状態で何が起きるか。3つの立場から整理します。

**デザイナーの視点。** 画面を作るたびに、ボタンの角丸は何pxだったか、エラーの赤はどの赤だったかを過去のファイルから探す作業が発生します。見つけた参照元が古い画面で、すでに別の画面で更新されていた——こうしたズレが積み重なり、同じプロダクト内で微妙に異なるUIが増殖します。新しくジョインしたデザイナーは、どのファイルが正なのかわからず、先輩に都度確認するか、自分の判断で作るかのどちらかになります。

**エンジニアの視点。** デザインカンプが届くたびに、この余白は前回と同じ意図なのか、この色は既存のトークンに対応するのかを推測する必要があります。同じ見た目のボタンが画面ごとに異なるCSSで実装され、あとから一括変更したいときに影響範囲が把握できない。新機能のたびに既存UIの再発明が起き、似たようなコンポーネントが複数のディレクトリに散らばります。

**PdMの視点。** 新機能の見積もりに「UIの共通化が追いついていないので工数が余分にかかる」という注釈がつくようになります。リブランドやトーン変更の要望が出たとき、影響範囲の洗い出しだけで数週間かかることが判明する。デザインレビューでは細部の指摘（ここの間隔が違う、この色が合っていない）に時間が使われ、本来議論すべきユーザー体験の話に到達しない。

いずれの立場でも、共通しているのは判断の繰り返しです。すでに決まっているべきことが決まっていないために、毎回同じ判断をゼロからやり直す。デザインシステムは、この繰り返しを仕組みで吸収するためのものです。
