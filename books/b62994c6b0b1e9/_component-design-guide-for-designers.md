...
命名では否定形を避けます。Figmaでいえば、トグルスイッチの名前を「無効ではない」にするか「無効」にするかの違いです。`isNotDisabled` ではなく `isDisabled`。二重否定は読み手を混乱させるだけです。

また、動詞で始めるのが一般的です。`is` は状態（`isDisabled`）、`has` はあるかないか（`hasBorder`）、`show` は表示するかどうか（`showIcon`）を表します。
