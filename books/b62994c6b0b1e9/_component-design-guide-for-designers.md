...
この性質を説明するのが経路依存性です。タイピング教育のカリキュラム、OSのデフォルト設定、キーボード製造の金型、オフィスの研修マニュアル——電子キーボードが生まれた時にはすべてがQWERTYを前提に構築されていました。ひとつの要素だけを変えても周辺が追いつかず、全部を同時に変えるのはコストが膨大すぎる。初期の技術的制約で決まった選択が、制約が消えた後も社会全体のデファクトスタンダードであり続け、加速度的な定着を繰り返します。
...
コンポーネント設計の具体に入る前に、デザインシステムの構築戦略のパターンを概観します。以下の戦略を比較します。
...
Figmaでたとえるなら、マスターコンポーネントに新しいバリアントを追加するのは自由（＝拡張に開かれている）ですが、すでにあるバリアントのプロパティ名を変えれば全インスタンスに影響します（＝修正には閉じられているべき）。まさにこの記事で繰り返してきた「追加は低リスク、変更は高リスク」を一般化した原則です。
...
- **色の制御** — 使ってアイコンの色を自動で合わせるしくみを取り入れ、個別に画像の用意を不要にする
