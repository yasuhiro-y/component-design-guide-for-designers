...
デザイナー向けに書いていますが、エンジニアやPMにも役立つ内容を含んでいます。エンジニアにとっては、デザイナーの観点からの設計トピックを概観することができ、デザイナーにエンジニアリングならではの概念を説明することに役立ちます。経営者やPdMにとっては、デザインシステムへの投資がなぜ開発速度やプロダクト品質につながるのか、その構造的な理由が見えるようになります。
...
UIデザインをしてきたデザイナーであれば「ああこれね」となるパートも多いと思います。感覚的に理解していることをあえて言語化しているにすぎない箇所もあります。

デザインシステムにこれから取り組むチームであれば、職種を問わずこの記事を共通のリファレンスとして読んだうえで議論を始めると、前提知識のすり合わせやキャッチアップにかかる時間を大きく短縮できるはずです。
...
ただし、AIの対応力が向上すれば、こうした設計知識がなくても困らない場面は増えていくかもしれません。とはいえ、構造的な設計のメンタルモデルはAIのためだけのものではありません。開発コミュニケーションやドメイン知識の継承にも、これまで大きく貢献してきたものです。
...
まず6つの原則をざっくり紹介します。聞き慣れない用語もありますが、それぞれの節で丁寧に扱うので、ここでは全体の地図として眺めてください。

- Figmaのマスターコンポーネントを直せば、すべてのインスタンスに反映される。この「一箇所直せば全部直る」仕組みを**間接化**と呼ぶ
- ボタンの裏にある数百の設計判断を隠し、使う側には「variantとsizeを選ぶだけ」にしてくれる。これが**カプセル化**
- 色を1680万色から10色に絞る、余白を4の倍数に限定する。選択肢を減らして迷いをなくすのが**制約**
- 見た目がそっくりなモーダルでも「閉じられるダイアログ」と「確認を強制するダイアログ」は別物。見た目ではなく意味で分けるのが**意味の符号化**
- Avatar、Text、Badge、Button。4つの小さな部品を並べ替えるだけでUserCardにもNotificationItemにもなる。これが**合成**
- `primary` / `secondary` の次に何を足すか。命名規則があれば`destructive`、なければ`red-button`が生まれる。ルールを明文化するのが**慣習**
...
レゴブロックと同じです。同じブロックの組み合わせで、家もロボットも作れます。
...
拡張時に参照する体系をルール化するだけでなく、実装やFigmaファイルから類推できる状態を目指す。これはコンポーネントに限らず、トークンの命名体系、Enumの選択肢、ディレクトリ構造、あらゆる設計判断に通じる問いです。
...
デザインシステムでもまったく同じことが起こります。Figmaでたとえるなら、最初に`Auto Layout`なしで画面を作り始めてしまい、50画面できたところで`Auto Layout`に移行しようとしたら、全画面を作り直す必要が生じ、結局対応できないまま51画面目が生まれるという状況。過去の何気ない選択が、仕切り直しの機会すら奪ってしまいます。
...
慎重に追加するとは、未来を完璧に予見することではありません。完璧な予見はできないからこそ、後から改修しやすい構造を選んでおく。あるいは散らかったままでも意図的にそう判断したのだと記録しておく。
...
**エンジニアの視点。** デザインとコードの間に共通の語彙がないため、デザインカンプの解釈が毎回発生します。同じ意図の見た目が画面ごとに異なる実装として積み重なり、プレゼンテーション層に技術的負債が蓄積します。コードベースの中で似た役割のコンポーネントが散在し、どれが本体で何が派生なのかを把握するコストが機能追加のたびに増大します。
...
なんとなく共通化する、なんとなく専用のものを作る。この無自覚さが、経路依存性を通じて後から修正しにくい構造を生みます。いま自分たちがこのスペクトラムのどこにいるかを自覚し、意図的に選ぶ。この視点は以降の章で繰り返し登場します。
...
しかしジレンマがあります。この装置を構築すること自体がまだ答えのない問いに取り組む行為であり、創造的なリソースを消費します。デザインシステムを作る時間が、本来それが生み出すはずの余裕を食い潰しているのではないか。
...
もうひとつ、見落とされがちな視点があります。デザインシステムは「作るか作らないか」だけの問題ではありません。
...
デザインシステムが軌道に乗るかどうかは、コンポーネントの品質だけでなく、こうした受け入れ体制や意思決定フローで決まる部分が大きい。Figmaのライブラリを整えてもSlackでの相談先が不明なら使われない。完璧なコンポーネントを揃えてもリクエストへの応答が遅ければチームは独自に作り始めます。

この記事ではコンポーネントの設計にフォーカスしていますが、それが組織の中でどう流通し、どう進化していくかという運営の設計も同じくらい重要です。

原則を押さえたうえで、次はこれらの原則をどのような手段で実現するか、構築戦略の選択について検討します。

# 構築戦略: どう作るかを選ぶ

コンポーネント設計の具体に入る前に、デザインシステムの構築戦略のパターンをおさえます。構築方式の選択は、その後のデザインの自由度やチームの開発効率を大きく左右します。
...
最後に、戦略を選ぶ際のボトルネックの見極め方を扱います。

## 構築戦略の全体像

コンポーネントの設計論の序盤になぜこのような俯瞰的なトピックを扱うかというと、コンポーネントの運用は、土台となるデザインシステムの方式に大きく左右されるからです。

プロダクトの立ち上げ時に構築方法が入念に検討されることは稀で、限られた情報と状況の中で決まることがほとんどです。デザインシステムより優先すべき事情が山積みなのが現実です。そして、ある構築方法を選んだ日が、2年後のリブランドの可否を決めていた。そんなことが起こりえます。構築戦略の選択は、その後の自由度や拡大効率を大きく左右します。

もしこの記事をプロダクトの立ち上げ期に読んでいるデザイナーがいるとしたら、とても幸運なことでしょう。
...
もうひとつ見落とされがちな問題が、選択肢が多すぎることです。汎用ライブラリはあらゆるサービスで使えるように設計されているため、ボタンひとつとっても膨大なプロパティが用意されています。たとえばブランドガイドラインでプライマリボタンの色が決まっていても、ライブラリ上では`colorScheme`に好きな色を指定できてしまいます。`size`もxsからxlまで5段階あるが、自社で使うのは2つだけ。`variant`も10種類あるが、プロダクトで許容しているのは3種類。
...
もうひとつ実務で地味に効いてくるのが、ライブラリのデフォルトの挙動が自社の方針と合わないケースです。たとえば`Disabled`状態のボタンをライブラリは`opacity: 50%`で表現するが、自社では背景色をグレーに変えたい、といった不一致はよくあります。

多くの汎用ライブラリにはテーミング機能があり、ある程度はオーバーライドで対応できます。しかしライブラリの設計思想と根本的にぶつかる場合は、その部品だけ自前で作る判断が必要になることもあります。こうした使えるところは使い、合わないところは割り切る、という判断は、後述するハイブリッド戦略につながる考え方です。
...
いくつかのパターンを紹介しましたが、現実的にはこれらを組み合わせることが有力です。汎用ライブラリだけで完結するサービスはほとんどなく、プロダクトが育つにつれて、チャット画面やドメイン固有のダッシュボードなど、汎用ライブラリにはないUIが必ず生まれます。
...
組織が複数のプロダクトを持つ場合は、この線引きがさらに重要になります。あるプロダクトはChakra UIを、別のプロダクトはShadcn/uiを使っている。こうした状態では、`Alert`と`Banner`のような名前のズレがプロダクト横断のコミュニケーションを混乱させます。

理想的には汎用ライブラリを全プロダクトで統一することです。これから新しいプロダクトを立ち上げるなら、既存プロダクトと同じライブラリを選ぶことを強く推奨します。すでに異なるライブラリで作り込んでいる場合は、無理にプロダクト横断の独自体系を作るより、各プロダクトが採用しているライブラリの流儀に合わせるほうが現実的です。

一方、デザイントークン（色・余白・フォントサイズ）やアイコンセットは、ライブラリが違っても共通化しやすく、コストパフォーマンスの高い統一ポイントになります。
...
構築戦略を決める前に、自分たちのチームで何が本当のボトルネックなのかを診断します。従来、ボトルネックはUIの実装スピードにありました。しかしAIがコードを書く時代には、事情が変わりつつあります。

実装スピード以上に、決定の質がレバレッジを持つようになってきています。良い決定をするために必要な情報がチーム内で時間内に集まるか、変更の影響を正確に把握できるか。こうしたフィードバックループの速度が、決定の質を左右します。
...
構築戦略が土台を決め、次に問われるのはその土台の上に、どんな部品をどう分けて並べるかです。
