...
※ このあたりからエンジニアリングの用語を混ぜて書いていますが、後のパートで解説がありますので、読み進めてください。
...
**プロパティの拡張パスを確保する。** Enumの選択肢が将来増えることを想定し、現時点では1つしか選択肢がなくても、スケール上の位置を明示しておく。`default` ではなく `medium` と名付ける（変数の章のTシャツサイズの節で扱いました）。Booleanで始めても構いませんが、3つ目の選択肢が浮上した瞬間にEnumへ移行する覚悟を持っておく。
