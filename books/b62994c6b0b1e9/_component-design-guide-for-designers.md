...
これら6つは独立したチェックリストではなく、互いに補強し合い、ときにトレードオフを生む体系です。こうした絶対的な正解のなさがデザインシステムを難しくしている要因であり、奥深さです。
...
拡張するために参照する体系をルール化することに加え、実装やFigmaファイルから類推可能であること。これはコンポーネントに限らず、トークンの命名体系、Enumの選択肢、ディレクトリ構造、あらゆる設計判断に通じる問いです。
...
理想の体制がすぐに敷けるとは限りません。走り出してから後でまとめて直すほうが効率的なこともあります。
...
なんとなく共通化する、なんとなく専用のものを作る。この無自覚さが、経路依存性を通じて後から修正しにくい構造を生みます。いま自分たちがこのスペクトラムのどこにいるかを自覚し、意図的に選ぶ。この問いは以降の章で繰り返し登場します。
...
コンポーネントの設計論の序盤になぜこのような俯瞰的なトピックを扱うかというと、コンポーネントの運用は土台となるデザインシステムの方式に規定される部分が大きいからです。

しばしば、プロダクトが生まれる際、それをどう構築するかの検討は入念におこなわれなかったり、限られた情報と特有の状況のなかでおこなわれたりします。デザインシステム以上に重要と思われる事情がたくさんあることのほうが普通です。そして、ある構築方法を選んだ日が、2年後のリブランドの可否を決めていた。そんなことが起こりえます。構築戦略の選択は、その後の自由度や拡大効率を大きく左右します。

もしこの記事をプロダクトの立ち上げ期に読んでいるデザイナーがいるとしたらとても幸運なことでしょう。
...
一方で、すでに稼働中のプロダクトにデザインシステムを導入するのであれば、機能や挙動の本質を変えずにパーツを差し替えていくことであるので、LLMによる実装の再現性が比較的高い領域です。デザインシステムのコンポーネント定義がコードベースに存在していれば、LLMはそれを読み取り、既存のコンポーネントを使った実装を生成できます。定義がなければLLMは独自の解釈で実装するため、既存コードとの一貫性は保たれません。
...
実装スピード以上に、決定の質がより大きなレバレッジを持つようになってきていると考えます。良い決定をするために必要な情報がチーム内で時間内に集まるか、変更の影響を正確に把握できるか。こうしたフィードバックループの速度が、決定の質を左右します。
...
Atomic Designは日本のサービスでも導入事例が豊富であり、そして課題やその解決策も多く発信されています。
...
たとえば、求人サイトで`Button`コンポーネントを汎用的に扱うのであれば、「履歴書不要」という文言をコンポーネントに直接埋め込むのではなく、`label` プロパティを通じて外からテキストを渡す設計にします。`<Button label="履歴書不要で応募する" />` のように使えば、同じButtonコンポーネントが「ログイン」にも「購入する」にも使い回せます。コンポーネントの設計の中にドメイン知識が入り込んでいる状態は避けるべきです。
...
フロントエンドやモバイルの実装では、コンポーネント名でコンポーネントを呼び出します。最初につけた名前がそのままコードに入り、ドキュメントに引用され、チームの会話に定着します。名前を変更するということは、すべての呼び出し箇所を修正するということです。後から変えるコストは思った以上に高いので、一度決めたらなるべく変えないぐらいの気持ちで設計します。議論になったことはその場でルール化してドキュメントに残しておかなければ、同じ議論や調整が繰り返されることになります。

`Frame 1` や `Property 1` のような命名でFigmaデータを作っても、コード側では必ず命名がおこなわれます。であれば、デザイナーが意味や意図をもとに命名に参加することは意義深いはずです。
...
増やすのは安全、変えるのは大変。ソフトウェア設計にはこれに近い考え方として開放閉鎖の原則（OCP: Open-Closed Principle）があります。拡張には開かれているが、修正には閉じられているべき、という設計指針です。Figmaでたとえるなら、マスターコンポーネントに新しいバリアントを追加するのは自由（＝拡張に開かれている）ですが、すでにあるバリアントのプロパティ名を変えれば全インスタンスに影響します（＝修正には閉じられているべき）。まさにこの記事全体で繰り返してきた追加は安全、変更は危険を一般化した原則です。
...
意味で名付けることの価値は意味の符号化の節で述べたとおりです。しかしプロダクトの初期段階では、意味自体がまだ定まっていないことがあります。無理に意味を固定するよりも、序列で仮置きしておき、パターンが見えてきた段階で意味ある名前に昇格させるほうが、経路依存性のリスクを抑えられます。
...
これを使いなさいと指示するだけの規範的なシステムより、これを組み合わせて新しいものも作れると可能性を開く生成的なシステムのほうが、こうした進化を受け入れやすくなります。デザインシステムは石碑ではなく生態系です。
...
認識が共有されていないとFigmaと実装でプロパティの数が合わないという無駄な議論が繰り返されるためです。
...
ここまで、コンポーネントが受け取る設定値であるプロパティの型と組み合わせを扱いました。プロパティが何を受け取るかなら、状態はいまどう見えるかです。
...
この方式は余白の原則と完全に一致しています。コンポーネントは自分の中身だけに責任を持ち、要素間の装飾は親が制御する。そして、色々な要素が登場してもそれぞれごとにborderをつける必要がありません。ただし、配置のたびに`Divider`を挟む手間が増えるため、`showDivider`プロパティで内包する方式とはトレードオフの関係です。チームの方針として統一しておくのがよいでしょう。
