...
コンポーネントというトピックひとつとってもプラクティスがたくさん存在します。ところが、その原則や経験則がないと、何がわからないかわからない（Unknown unknowns）な状態です。たとえば、色を何種類まで許容するか、ボタンのサイズは何段階必要か、コンポーネント名を変えたときにコード側で何が起きるか——こうした問いがあること自体を知らなければ、調べようがありません。
...
とはいえ、UIデザインをしてきたデザイナーであれば感覚的に理解していることをあえて言語化しているにすぎない箇所もあります。読んでいただくとわかるかと思いますが、ああこれね、となるパートも多くあると思います。一度体得されるとあえて文章化されないような事柄がたくさんある事に気づきます。
...
ただし、AIのよしな力が高まれば、人間が苦心する場面も減っていくかもしれません。これはAIのためだけではなく、これまでも人間による開発コミュニケーションや実装・ドメイン知識の継承に大きく寄与するメンタルモデルでした。
...
この一言では、あまりに表現できないことが多すぎます。分解しましょう。
...
この論点はチームの規模やフェーズで最適解が変わるため、ここでは概要に留めます。どちらが優れているという話ではなく、短期的な最適解と長期的な最適解が異なることが多い点がポイントです。立ち上げ期は分散型で素早く動き、プロダクトが安定してきたら集権型に寄せていく。あるいはその逆もあり得ます。自分たちの組織の規模やフェーズに合わせて選ぶものであり、理想の体制がすぐに敷けるとは限りません。とりあえず走り出して後でまとめて直したほうが効率がよいということもあります。
...
もちろん、「バッジを載せたい」「横並びにしたい」という具体的な要件を初日に予見できたかは別の問題です。ここでの教訓は「未来を完璧に読め」ではなく、「差し込み口やレイアウト切り替えの余地といった**構造的なゆとり**は、具体的な要件が見えていなくても仕込める」ということです。中身は後から埋めるが、器の形だけは先に決めておく——この考え方は、後述する「不確実性が高いときの構造設計」（現実的な問題への対処の章）でより詳しく扱います。
...
この性質を知っていると、「とりあえず追加しておこう」という判断の重みが変わります。追加はほぼ無料に見えますが、実際にはその瞬間から将来の削除コストが積み上がり始めています。簡単に増やせるからこそ、増やすときにこそよく考える必要があります。慎重に追加し、使われなくなったものは早めに非推奨（deprecated）にする。この姿勢が、長く使えるコンポーネントライブラリを支えます。

この「慎重に追加する」という姿勢は、未来を完璧に予見することとは違います。完璧な予見はできないからこそ、後から改修しやすい構造を選んでおく。あるいは散らかったままでも意図的にそう判断したのだと記録しておく。チームの状況に応じた落としどころを探ることになります。
...
**ブランドの信頼性。** 一貫したデザインは、プロダクトがきちんと作られているという印象を与えます。画面ごとにフォントサイズが微妙に違う、ボタンの角丸がページによって異なる——ユーザーはこうした不揃いを意識的に認知せずとも「なんとなく雑だ」と感じます。逆に、細部まで統一されたプロダクトは、その背後にある意図と規律を無言で伝えます。ブランドの信頼は、派手なビジュアルではなく、こうした静かな一貫性から醸成されます。

**再利用性。** 上記の原則が示してきたように、一貫性は再利用の前提条件でもあります。コンポーネントの設計が一貫していなければ「使い回せる部品」は成立しません。命名がバラバラ、プロパティの型がコンポーネントごとに異なる、同じ概念に別の名前がついている。こうした不揃いは再利用のコストを上げ、間接化のレバレッジを弱めます。
...
一貫性の目的は均一にすることではありません。基準（ベースライン）を確立することです。基準があるからこそ、そこからの意図的な逸脱がシグナルとして機能します。余白がひときわ広い箇所は「ここで視線を止めてほしい」という意思表示になり、普段使わない色が現れれば「これは通常とは異なる操作です」と伝わります。デザインにおけるコントラストや視覚的階層（ビジュアルヒエラルキー）は、この原理の上に成り立っています。
...
**マネージャーの視点。** プロダクトの成長とともに、UI開発の速度が鈍化し、デザイン品質を統一的に保つ難易度も上がっていきます。画面数が増えるほど暗黙の規約も増え、それを知っている人がボトルネックになる。チーム横断でUIの一貫性を保つには、システムによる担保ではなく人による調整が必要になり、コミュニケーションコストがチームの人数に比例して膨らみます。デザインシステムへの投資判断を先送りするほど、あとから導入するコストが上がっていく——これも経路依存性の一種です。
...
ただし、デザインシステムの導入がつねに正解というわけではありません。立ち上げ期のプロダクトでは、UIの方向性自体がまだ固まっていないことがあります。その段階で体系化に投資すると、試行錯誤の速度を落とし、まだ見えていない要件に対して早すぎる最適化をしてしまうリスクがあります。少人数のチームでは暗黙の共有で十分に回っている場合もあります。仕組み化のコストが赤字になって回収できないおそれもあります。
...
しかし、ここにひとつのジレンマがあります。デザインシステムは定型判断をシステムに委ね、人間の注意力をまだ答えのない問題に向ける装置です。ところが、その装置を構築すること自体が「まだ答えのない問い」に取り組む行為です。どんなコンポーネントが必要になるか、どの粒度で分割すべきか。こうした問いに向き合うこと自体が創造的なリソースを消費します。デザインシステムを作る時間が、本来それが生み出すはずの余裕を食い潰しているのではないか。このジレンマは実務で常に付きまといます。

答えは「作るな」でも「完璧に作ってから始めろ」でもありません。問われているのは「いま、どの時間軸にリソースを配分するか」という判断です。短期の価値創出を優先すべき局面もあれば、構造に投資して中長期の自由度を確保すべき局面もあります。この配分は一度決めたら固定ではなく、プロダクトのフェーズとともに変わります。原則を知っていることの価値は、完璧な設計ができることではなく、いまどこに妥協しているかを自覚し、後から立ち戻る道筋を残せることにあります。
...
コンポーネントの設計論の序盤になぜこのような大仰なトピックを扱うかというと、コンポーネントの運用は土台となるデザインシステムの方式に規定される部分が大きいからです。
...
多くの汎用ライブラリにはテーミング機能があり、ある程度はオーバーライドで対応できます。しかしライブラリの設計思想と根本的にぶつかる場合は、その部品だけ自前で作る判断が必要になることもあります。こうした「使えるところは使い、合わないところは割り切る」判断は、後述するハイブリッド戦略につながる考え方です。
...
一方で、すでに稼働中のプロダクトにデザインシステムを導入するのであれば、機能や挙動の本質を変えずにパーツを差し替えていくことであるので、LLMによる実装の再現性が比較的高い領域です。デザインシステムのコンポーネント定義がコードベースに存在していれば、LLMはそれを読み取り、既存のコンポーネントを使った実装を生成できます。定義がなければLLMは独自の解釈で実装するため、既存コードとの一貫性は保たれません。見た目とロジックが分離されていれば、表層の差し替えとして作業を進め、画面で確認・デバッグしながら品質を担保できます。
...
構築戦略が土台を決め、次に問われるのは「その土台の上に、どんな部品をどう分けて並べるか」です。
...
なぜデザイナーがこの区別を知る必要があるのか。この境界を意識しないと「なんでも入る万能コンポーネント」か「どこにも使い回せない専用コンポーネント」の両極端に振れやすいからです。汎用コンポーネントはどこからでも呼び出せるが、ドメインコンポーネントは特定の機能の文脈でのみ使う——この規約を設けることで、プロパティの肥大化と亜種の増殖を同時に防げます。
...
エンジニアがよく口にする「後方互換性」（Backward Compatibility）とは、まさにこの話です。後方互換性とは、**部品を新しくしても、それを使っている既存の箇所を修正しなくて済む**という性質です。USB-Aの端子が何年経っても形を変えないおかげで古いケーブルがそのまま挿せるように、インターフェースが安定していれば、利用者はアップデートを恐れずに済みます。
...
増やすのは安全、変えるのは大変。ソフトウェア設計にはこれに近い考え方として開放閉鎖の原則（OCP: Open-Closed Principle）があります。拡張には開かれているが、修正には閉じられているべき、という設計指針です。Figmaでたとえるなら、マスターコンポーネントに新しいバリアントを追加するのは自由（＝拡張に開かれている）ですが、すでにあるバリアントのプロパティ名を変えれば全インスタンスに影響します（＝修正には閉じられているべき）。まさにこの記事全体で繰り返してきた「追加は安全、変更は危険」を一般化した原則です。コンポーネント設計でも、新バリアントの追加は歓迎しつつ、既存プロパティの変更は慎重に——という姿勢に通じます。この考え方は、変数(1)で扱うBoolean→Enum移行にも通じます。

### 不確実性が高いときの構造設計: 器の形だけ先に決める

追加が安全で変更が危険であるなら、まだ何が追加されるかわからない段階では、将来の追加を受け入れやすい構造をあらかじめ敷いておくことが防御策になります。

これは「完璧な設計を最初から決める」とは真逆の発想です。未来が読めないからこそ、意図的に構造のゆとりを残しておく。

**意味論を弱くして破綻を防ぐ。** たとえばバリアントの命名で `callToAction`・`confirmation`・`navigation` のように具体的な用途で名付けると、その用途に当てはまらない新しいボタンが登場した瞬間に体系が崩れます。`primary` / `secondary` / `tertiary` のような序列型の命名は、意味論的には弱い——何が`primary`で何が`secondary`かは文脈次第です。しかし、まさにその曖昧さゆえに、新しいユースケースが現れても名前が矛盾しません。4つ目が必要になれば`quaternary`を足すか、別の体系に切り替える判断をする余地が残ります。

意味で名付けることの価値は「意味の符号化」の節で述べたとおりです。しかしプロダクトの初期段階では、意味自体がまだ定まっていないことがあります。無理に意味を固定するよりも、序列で仮置きしておき、パターンが見えてきた段階で意味ある名前に昇格させるほうが、経路依存性のリスクを抑えられます。
...
階層を固定しないまま場当たりで増やすと、`error-red` / `feedback.success.background` / `warningColor` のように深さも命名規則もバラバラなトークンが混在します。後から体系化しようとすると、参照箇所すべてを書き換える破壊的変更になります。中身の値は後で変えられますが、階層構造の変更は影響が大きい。だからこそ、構造だけは早めに固めておく価値があります。
...
1ヶ月後、リストにエラー表示が必要になります。ボーダーを赤くする `error` スタイルを追加——ここまでは問題ありません。ところが、「注目アイテムなのにエラーでもある」というデータが出現します。薄い青の背景に赤いボーダー。見た目はどう見ても変です。
...
もちろん現実には、もっと早い段階で手を打てます。「注目」を別のコンポーネントに切り出す、デザイン段階で状態の掛け合わせを減らす、時間をかけて構造ごと直す——いずれも有効な選択肢です。

この例えの教訓は「場当たり対応をするな」ではありません。締め切りがある以上、やらざるを得ない場面は往々にしてあります。問題は、1つ目の応急処置が2つ目を、2つ目が3つ目を呼ぶ連鎖に気づけるかどうかです。「あれ、これ前も似たようなことやったな」と思った時点で、根本を直す判断ができるかどうか。その嗅覚は、状態の掛け合わせを普段から意識しているかどうかで大きく変わります。
...
実際に並べてみると、どのライブラリも Button・Input・Select・Tag・Switch・Checkbox といった基本部品を揃えており、それぞれが `size`・`variant`・`colorScheme`・`disabled` といった類似のプロパティを持っています。名前やAPIの細部は違っても、設計の骨格は共通しています。これは偶然ではなく、UIの問題構造が同じだからです。ボタンには見た目のバリエーションとサイズが必要で、入力フィールドにはプレースホルダーと無効状態が必要——こうした要件は言語やフレームワークを問わず共通します。だからこそ、ここから先で扱う変数の型や設計パターンは、特定のライブラリに依存しない普遍的な知識として使えます。
...
ここまで、コンポーネントが受け取る設定値であるプロパティの型と組み合わせを扱いました。プロパティが「何を受け取るか」なら、状態は「いまどう見えるか」です。
...
デザインシステムに込められた設計判断の一つひとつは、チームが「良いデザインとは何か」という問いに対して積み重ねてきた回答の集積でもあります。余白のリズム、色の意味の体系、インタラクションの質感。新しいメンバーがコンポーネントライブラリを使い始めるとき、そこには先人が磨き上げた美意識やクラフトマンシップが宿っています。デザインシステムは効率化の装置であると同時に、デザイン文化の伝達媒体です。
