...
| 型 | Figmaでの表現 | コードでの書き方（参考） | Swift | Kotlin |
|---|---|---|---|---|
| Boolean | トグルスイッチ | `isDisabled: boolean` | `Bool` | `Boolean` |
| Enum | バリアント（Variant） | `size: 'S' \| 'M' \| 'L'` | `enum` | `enum class` |
| String | テキスト入力 | `label: string` | `String` | `String` |
| Number | テキスト入力 | `count: number` | `Int` / `Double` | `Int` / `Double` |
| Date | — | `deadline: Date` | `Date` | `LocalDate` / `Instant` |
| Array | Auto Layout内の繰り返し | `items: Item[]` | `[T]` | `List<T>` |
| Object | 複数プロパティの集合 | `user: User` | `struct` | `data class` |
| Element | `Instance Swap` | `icon: ReactNode` | `some View` | `@Composable` |
