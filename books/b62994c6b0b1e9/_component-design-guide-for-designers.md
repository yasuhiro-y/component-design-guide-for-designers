...
デザインシステムやコンポーネント志向の体得は、デザインディレクションとソフトウェアエンジニアリングの接近へ挑戦です。プロパティの設計、命名規則、変更の安全性、関心の分離。これらはエンジニアが日常的に考えていることです。デザイナーがこうした考え方を身につけると、3つの変化が起こります。
...
[経路依存性](https://ja.wikipedia.org/wiki/%E7%B5%8C%E8%B7%AF%E4%BE%9D%E5%AD%98%E6%80%A7)（path dependency）とは、初期に選んだ道がその後の選択肢を制約し、後戻りが難しくなる現象のです。
...
時は経ち、電子キーボードの時代。アームのないキーボードが普及しはじめました。ところが、こうして機械的な制約はもう存在しなくなったもかかわらず、配列はそのまま残っている。私たちの多くはQWERTY配列のキーボードを使っています。
...
一貫性の目的は均一にすることそのものよりも、デザイナーが操作する意味が生まれる前提となる基準（ベースライン）を確立するとであると考えることができます。
...
この考え方の背景にあるのがSSOT（Single Source of Truth）、真実の情報源はひとつだけという原則です。原則の章で紹介した間接化の考え方そのものです。Figmaでいえば、コンポーネントのマスターを1つ持ち、全画面でそのインスタンスを使う。マスターを更新すれば全インスタンスに反映される。
...
ここまで、コンポーネントの分割・分類・命名という設計のルールを整理してきました。次の章では、設計したルールが現実の制約や変化にぶつかったとき、どのように対処し、どこまで妥協するかという問いを扱います。
...
ここまで、コンポーネントが受け取る設定値であるプロパティの型と組み合わせを扱いました。プロパティが「何を受け取るか」であるのに対し、状態は「いまどう見えるか」です。
