...
このことから、汎用コンポーネントは特定のビジネスロジック（業務固有のルールや判断）を内包しないのが原則です。たとえば、求人サイトで`Button`コンポーネントを汎用的に扱うのであれば、「履歴書不要」という文言をコンポーネントに直接埋め込むのではなく、`label` プロパティを通じて外からテキストを渡す設計にします。`<Button label="履歴書不要で応募する" />` のように使えば、同じButtonコンポーネントが「ログイン」にも「購入する」にも使い回せます。コンポーネントの設計の中にドメイン知識が入り込んでいる状態は避けるべきです。
