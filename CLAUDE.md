...
- 原稿内の章見出しは `# タイトル` (H1)。split 時に見出しレベルはそのまま維持される（H2, H3, H4 がそのまま出力される）。
...
- バッククォート（`` ` ``）はプロパティ名・型名・プロパティ値・CSS プロパティなどのコード要素に使う。コンポーネント名（Button, Card, Dialog 等）にはバッククォートを付けない。一般的な概念や用語にも使わない。
